...
def test_general_products_seed_has_ten_items_and_five_variants() -> None:
    mistral_products = [
        product for product in GENERAL_PRODUCTS if product.get("presentation_family") == "Pisco Mistral"
    ]
...
def test_seed_general_recreates_general_catalog_from_scratch() -> None:
    ...
    db_mock.query.side_effect = [category_query] + [product_query] * len(GENERAL_PRODUCTS)
